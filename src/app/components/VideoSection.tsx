...
function VideoCard({ video, index }: { video: (typeof videosData.videos)[0]; index: number }) {
  ...
  return (
    <div
      ...
      className={`glass-card rounded-3xl overflow-hidden group transition-all duration-700 hover:border-primary/30 hover:-translate-y-2 hover:shadow-2xl ${
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className={`video-play-btn transition-all duration-300 ${hovered ? 'scale-125 shadow-lg shadow-primary/30' : 'scale-100'}`}>
      ...
      <div className="p-5 space-y-2">
        <h3 className={`text-sm font-700 leading-tight transition-colors duration-300 ${hovered ? 'text-primary' : 'text-foreground'}`}>{video.title}</h3>
        <p className="text-xs text-muted-foreground leading-relaxed line-clamp-2">{video.description}</p>
        <div className="flex items-center justify-between pt-1">
          <span className="text-xs text-muted-foreground">{video.views} views</span>
          <button className={`text-xs font-600 hover:text-primary/80 transition-all duration-300 flex items-center gap-1 ${hovered ? 'text-primary translate-x-1' : 'text-primary'}`}>
        ...
        <div className="reveal delay-250 flex flex-wrap gap-2 justify-center mb-10">
          {videosData.categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-700 uppercase tracking-wider transition-all duration-300 min-h-[44px] ${
                activeFilter === cat
