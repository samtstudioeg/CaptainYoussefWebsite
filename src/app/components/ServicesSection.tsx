...
function ServiceCard({ service, index }: { service: (typeof servicesData.services)[0]; index: number }) {
  ...
  return (
    <div
      ...
      className={`glass-card rounded-3xl overflow-hidden service-card-hover transition-all duration-500 flex flex-col ${
        visible ? 'opacity-100 translate-y-0' : 'opacity-100'
      }`}
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden shrink-0">
        ...
        {/* Icon */}
        <div className="absolute top-4 left-4 w-10 h-10 rounded-xl bg-primary/20 backdrop-blur-sm border border-primary/30 flex items-center justify-center">
      ...
      <div className="p-6 flex flex-col flex-1 space-y-4">
        <h3 className="text-lg font-700 text-foreground leading-tight">{service.title}</h3>
    ...
    <section id="services" className="py-24 px-6 relative">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">{servicesData.subtitle}</p>
        </div>

        {/* Services Grid — 4 cols on xl, 3 on lg, 2 on md, 1 on sm */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {servicesData.services.map((service, i) => (
            <ServiceCard key={service.id} service={service} index={i} />
          ))}
          {/* 7 cards: last row on xl has 3 cards in first row + 4 in second = no, 7 cards / 4 cols: row1=4, row2=3. Last card expands */}
        </div>
      </div>
    </section>
  );
