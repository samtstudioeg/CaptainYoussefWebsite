    ...
    <section id="contact" className="py-24 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative" ref={containerRef}>
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <p className="reveal delay-250 text-muted-foreground max-w-xl mx-auto">{contactData.subtitle}</p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 space-y-8 reveal-left">
            ...
            <div className="space-y-4">
              {contactData.options.map((opt, i) => (
                <a
                  key={opt.label}
                  href={opt.href}
                  className="flex items-center gap-4 glass-card rounded-2xl px-5 py-4 hover:border-primary/30 hover:translate-x-1 transition-all duration-300 group min-h-[44px]"
            ...
            <a
              ...
              className="flex items-center gap-3 w-full px-6 py-4 rounded-2xl bg-[#25D366]/10 border border-[#25D366]/30 hover:bg-[#25D366]/20 hover:scale-[1.02] transition-all duration-300 group"
            >
              <div className="w-10 h-10 rounded-xl bg-[#25D366]/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300">
              ...
              <Icon name="ArrowRightIcon" size={16} className="text-[#25D366] ml-auto group-hover:translate-x-2 transition-transform duration-300" />
            </a>
            ...
            <div className="flex gap-3 flex-wrap">
              {contactData.social.map((s, i) => (
                <a
                  ...
                  className="w-11 h-11 glass-card rounded-xl flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/30 hover:scale-110 transition-all duration-200"
            ...
            <div className="glass-card rounded-3xl p-8 space-y-5">
              {submitted ? (
                <div className="text-center py-12 space-y-4">
                  <div className="w-16 h-16 rounded-full bg-accent/10 border border-accent/30 flex items-center justify-center mx-auto animate-counter-pop">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {contactData.formFields.slice(0, 2).map((field) => (
                      <div key={field.name} className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:scale-[1.01] transition-all duration-200"
                        />
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={field.name} className="space-y-2">
                        ...
                        {field.type === 'select' ? (
                          <select
                            ...
                            required={field.required}
                            onChange={handleChange}
                            className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary focus:scale-[1.01] transition-all duration-200 appearance-none"
                          >
                  ...
                  {contactData.formFields.slice(4).map((field) => (
                    <div key={field.name} className="space-y-2">
                      ...
                      <textarea
                        ...
                        rows={5}
                        onChange={handleChange}
                        className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:scale-[1.005] transition-all duration-200 resize-none"
                      />
                    </div>
                  ))}
                  ...
                  <button
                    type="submit"
                    className="btn-primary w-full justify-center text-sm py-4 hover:scale-[1.02] transition-transform duration-200"
                  >
